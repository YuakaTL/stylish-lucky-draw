...
describe(`POST ${apiEndpoint}`, () => {
    ...
    it("should response with a 200 and ", async () => {
        ...
        if (process.env.USE_MOCK_DATA) {
            lotteryEvent = MOCK_DATA.mockAdminLotteryPostResponse;
    ...
    //* 測試項目2: error request body check
    it(`should response with a code ${CODE.queryRequiredError} if the request body  is not complete`, async () => {
        let error;
        input.event_start_time = undefined;
        if (process.env.USE_MOCK_DATA) {
    ...
    //* 測試項目6: 當 request body 的值型別錯誤時
    it(`should response with a code ${CODE.inputValueInvalidError} if the event_start_time is lower than current date`, async () => {
        let error;
        input.is_visible = "wrong type";
        if (process.env.USE_MOCK_DATA) {
